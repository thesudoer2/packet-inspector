...
void my_packet_handler(u_char *args, const struct pcap_pkthdr *packet_header, const u_char *packet_body) noexcept
{
    ...
    VERBOSE({
        ...
        auto hex_view = mac_addr_span | std::views::transform([](u_char c) { return std::format("{:02X}", c); })
                        | std::views::join_with(':');

        // Print the result
        std::string hex_string;
        for (const auto &item : hex_view) { hex_string += item; }
    ...
    if (ether_type == ETHERTYPE_IP) {
        ...
        if (packet_header->caplen < current_offset + sizeof(struct custom_ip_header)) {
            std::cerr << std::format("Packet too small for IP header! (Caplen: {}, Expected min: {})\n",
              ...
              current_offset + sizeof(struct custom_ip_header));
            return;
        }
        ...
        unsigned int ip_header_length =
          (ip_header->ip_vhl & 0x0F) * 4;// Length in 32-bit words, multiply by 4 for bytes
        ...
        if (ip_header->ip_p == IPPROTO_TCP) {
            ...
            VERBOSE(
              if (inserted) {
                  std::cout << "\t\t[NEW SESSION ESTABLISHED]\n";
                  session_manager->printSessions();
              } else {
                  std::cout << "\t\t[EXISTING SESSION]\n";
            ...
            if (application_data_len > 0) {
                ...
                DEBUG(
                  if (vlan_tag_len != 0) std::cout << std::format("\t\t\tVLAN/TAG Header Length: {}\n", vlan_tag_len));
                ...
                DEBUG(std::cout << std::format(
                        "\t\t\tTotal Layers Length (Layer 3 + Layer 4 + ... + Layer 7): {}\n", total_length));
                DEBUG(std::cout << std::format(
                        "\t\t\tTotal Packet Length (Layer 2 + Layer 3 + Layer 4 + ... + Layer 7): {}",
                        total_packet_length));
                DEBUG(if (vlan_tag_len != 0) std::cout << std::format(
                        " (+ VLAN/TAG Header Len ({}) = {})", vlan_tag_len, vlan_tag_len + total_packet_length);
                  std::cout << '\n');
                DEBUG(std::flush(std::cout));

                //////////////////////////////////////// Use DPI Engine (not implemented)
                ///////////////////////////////////////////////
                if (th_sport == 80 || th_dport == 80 || th_sport == 8080 || th_dport == 8080 || th_sport == 443
                    || th_dport == 443) {// HTTP Port
                    std::cout << std::format(
                      "\t\tTCP Application Layer: HTTP (likely) -- Source port: {}, Destination port: {}\n",
                      th_sport,
                      th_dport);
                    ...
                    VERBOSE(std::size_t bytes_to_print =
                ...
                VERBOSE(std::cout << std::format(
                          "\t\t[TCP Data Payload starts here, length {} bytes]\n", application_data_len));
            }
            ...
            if (application_data_len > 0) {
                VERBOSE(std::cout << std::format(
                          "\t\t[UDP Data Payload starts here, length {} bytes]\n", application_data_len));

                if (uh_sport == 53 || uh_dport == 53) {// DNS port
                    std::cout << std::format(
                      "\t\tUDP Application Layer: DNS (likely) -- Source port: {}, Destination port: {}\n",
                      uh_sport,
                      uh_dport);
                    ...
                    std::cout << std::format(
                      "\t\tUDP Application Layer: Unhandled Application Protocol (Source port: {}, Destination port: "
                      "{})\n",
                      uh_sport,
                      uh_dport);
                }
        ...
        std::flush(std::cout);
    } else if (ether_type == ETHERTYPE_ARP) {// Assuming ETHERTYPE_AARP and ETHERTYPE_LOOPBACK are correctly defined or
                                             // you add them
